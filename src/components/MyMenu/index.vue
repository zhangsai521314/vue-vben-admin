...
<template>
  <div :class="`${prefixCls}`" v-if="props.isDrawer" v-show="props.isShow">
    <a-drawer
      ...
      @close="props.cancel">
      <div :id="id" style="height: 100%;width: 100%;padding: 11px;margin: 5px;padding-top: 10px;border: 1px solid #f0f0f0;">
        <a-spin tip="数据点..." :spinning="isGetTree || isGetMenuInfo">
          <div style="width:100%" v-if="props.isShowSearch">
            <a-input-search v-model:value="searchContent" placeholder="搜索" :style="props.isEdit ? { width: 'calc(100% - 50px)' } : { width: 'calc(100% - 30px)' }" />
            <IconFontClass
              v-if="props.isEdit" name="icon-baseui-add" title="新增根菜单"
              :style="{ fontSize: '29px', position: 'relative', top: '-6px' }"
              @click="showMenuForm('add', node)" />
            <IconFontClass
              name="icon-baseui-redo" title="刷新菜单"
          ...
          <div :class="{ 'danXuan': props.isRadio }" style="width: 100%;height: 100%;">
            <a-tree
              ...
              <template #title="node">
                <!-- 节点插槽 -->
                <div :class="`${prefixCls}custom`" @mousemove="treeTitleMousemove" @mouseout="treeTitleMouseout">
                  ...
                  <span :class="`${prefixCls}option`" style="display: none;">
                    <IconFontClass
                      v-if="props.isEdit"
                      :class="{ 'not-click': !userData.isadmin && node.powertype.indexOf('3') == -1 && node.powertype.indexOf('1') == -1 }"
                      name="icon-baseui-tianjiawukuang" title="添加子设备"
                      style="color: #10893E"
                      @click="showMenuForm('add', node)" />
                    <IconFontClass
                      v-if="props.isEdit"
                      :class="{ 'not-click': !userData.isadmin && node.powertype.indexOf('2') == -1 && node.powertype.indexOf('1') == -1 }"
                      name="icon-baseui-edit-fill" title="编辑"
                      style="color: #0a61bd"
                      @click="showMenuForm('edit', node)" />
                    <IconFontClass
                      v-if="props.isEdit"
                      :class="{ 'not-click': node.children.length > 0 || (!userData.isadmin && node.powertype.indexOf('4') == -1 && node.powertype.indexOf('1') == -1) }"
...
function chenck(node) {
  if (!node.checked) {
    //选中
    if (props.isPageEditing && (!userData.value.isadmin && node.powertype.indexOf("1") == -1 && node.powertype.indexOf("2") == -1)) {
