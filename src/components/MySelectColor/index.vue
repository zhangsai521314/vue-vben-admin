...
<template>
  ...
  <div class="selectColorModel" ref="selectColorModelRef">
    <sketchs
      v-model="selectColorRef"
      @click="selectedColor" />
    <div class="divSelectColor">
      <a-button @click="saveColor" type="primary" style="float: right">关闭</a-button>
      <a-button @click="clearColor" style="float: right; margin-right: 2px; height: 33px">清空</a-button>
    </div>
  </div>
</template>
<script setup>
...
function showSelectColor() {
  $(".selectColor").remove();
  if (!props.disabled) {
    isShowSelectColor = domid != localStorage.getItem("clickSelectColorId") ? true : !isShowSelectColor;
...
onMounted(() => {
  $("body").click(function () {
    if (!$(event.target).hasClass("divColor") && !$(event.target).hasClass("selectColor") && $(event.target).parents(".selectColor").length == 0) {
      $(".selectColor").remove();
    }
  });
})
