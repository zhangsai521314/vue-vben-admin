    ...
    <a-spin tip="正在获取菜单..." :spinning="isGetMenuTree || isRunGetRightMenu">
              ...
              <template v-if="page.menutype == 'folder'">
                <div :class="`${prefixCls}view-title-folder`">
                  ...
                  <span
                    ...
                    :class="`${prefixCls}view-title-folder-level`">>返回上一级</span>
                  <span
                    :class="`${prefixCls}view-title-folder-level`"
                    v-for="(l, i) in page.folder.levels"
                    :key="i"
                    :title="l.title"
                    @click="rightClick_menu('open', page, l)">>{{ l.title }}</span>
                </div>
              </template>
              <template v-else>
      ...
      <div
        ...
        <div
          ...
          <div v-show="msgData.length > 0" :class="{ 'msg_div': msgData.length > 0 }">{{ msgData.length > 99 ? '99+' : msgData.length }}</div>
        </div>
        <!-- 显示桌面 -->
      ...
      <div
        ...
        <div :class="`${prefixCls}menu`">
          <a-menu
            @openChange="windowMenuOpenChange"
            theme="dark"
...
const desktopIcondivLineHeight = ref(desktopIconSize[userConfig.value.desktopIconSize].divLineHeight);
const desktopIcondivHeight = ref(desktopIconSize[userConfig.value.desktopIconSize].divLineHeight);
const desktopIcondivPaddingTop = ref(desktopIconSize[userConfig.value.desktopIconSize].divPaddingTop);
...
function rightOptions(mf, rightMenus, data, items) {
  ...
  mf.forEach((m) => {
    items.push({
      label: m.name,
      icon: m.icon,
      disabled: userData.value.isadmin ? false : (m.action == "delete" ? ((data.powertype.indexOf("1") != -1 || data.powertype.indexOf("4") != -1) ? false : true) :
        m.action == "edit" ? ((data.powertype.indexOf("1") != -1 || data.powertype.indexOf("2") != -1) ? false : true) : false),
      onClick: () => {
  ...
  switch (type_) {
        ...
        if (
          ...
          if (
            ...
            if (!myCommon.isnull(url)) {
              ...
              if (data.menuurl.length >= 5 && (data.menuurl.substring(0, 4).toLowerCase() == "http" || data.menuurl.substring(0, 5).toLowerCase() == "https")) {
    ...
    case 'edit':
      ...
      if (rig && !myCommon.isnull(rig.url)) {
        ...
        let url = rig.url;
        if (url.length >= 5 && (url.substring(0, 4).toLowerCase() == "http" || url.substring(0, 5).toLowerCase() == "https")) {
          url = url;
        } else {
          url = `#${url}/${data.key}`;
        }
...
function timeRefresh() {
  useTimeFn(setTimeout(() => {
    new_time.value = {
      time: dayjs().format('HH:mm'),
      date: dayjs().format('YYYY/MM/DD'),
    };
    timeRefresh();
  }, 1000), timeKey, "timeRefresh")
}
...
watch(
  ...
  () => {
    setTimeout(() => {
      ...
      desktopIcondivLineHeight.value = desktopIconSize[userConfig.value.desktopIconSize].divLineHeight;
      desktopIcondivHeight.value = desktopIconSize[userConfig.value.desktopIconSize].divHeight;
      desktopIcondivPaddingTop.value = desktopIconSize[userConfig.value.desktopIconSize].divPaddingTop;
